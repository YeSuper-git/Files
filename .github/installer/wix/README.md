...
The AV manager installer is built as a WiX Toolset 7 MSI wrapped by a Burn
bootstrapper:

- `Files-AV-Manager.msi` owns the application files, shortcuts, Explorer
  integration, identity-registration custom actions, repair, and uninstall.
- `Files-AV-Manager-Setup.exe` owns the user-facing setup, prerequisite chain,
...
reference. They are no longer used by the AV manager workflow.
